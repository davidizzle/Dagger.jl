...
struct ZeroMQ <: AbstractNetworkTransfer
    ...
    protocol::Protocol
    topic::String
    ZeroMQ(task=nothing; ip::IPAddr, topic::String) = new(task, Protocol(ip, 1883), topic)
    ZeroMQ(task=nothing; ip::IPAddr, port::Integer, topic::String) = new(task, Protocol(ip, port), topic)
end
=#
...
function stream_pull_values!(::RemoteFetcher, T, store_ref::Chunk{Store_remote}, buffer::Blocal, id::UInt) where {Store_remote, Blocal}
...
function stream_push_values!(::RemoteFetcher, T, store_ref::Store_remote, buffer::Blocal, id::UInt) where {Store_remote, Blocal}
    sleep(1)
end
...
function stream_push_values!(udp::UDP, T, store, buffer::Blocal, id::UInt) where {Blocal}
...
# Add ZeroMQ dispatch
# ZeroMQ dispatch
new_fetcher(zmq::ZerMQ) = ZerMQ(;ip=zmq.protocol.ip, port=zmq.protocol.port, topic=zmq.topic)
fetcher_task(zmq::ZerMQ) = zmq.task::DTask
function stream_push_values!(zmq::ZeroMQ, T, store, buffer, id::UInt)
    # values = _load_val_from_buffer!(buffer, T)
    context = Context()
...
function stream_pull_values!(zmq::ZeroMQ, T, store_ref::Chunk{Store_remote}, buffer::Blocal, id::UInt) where {Store_remote, Blocal}
